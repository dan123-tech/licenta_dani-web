...
export default function GloveboxVignetteViewerPage() {
  ...
  const url = payload?.active ? payload.car?.vignetteDocumentUrl : null;
  const ct = payload?.car?.vignetteDocumentContentType;
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col bg-slate-100">
      <header className="shrink-0 flex flex-wrap items-center gap-3 px-4 py-3 border-b border-slate-200 bg-white shadow-sm">
        ...
        <span className="text-sm font-semibold text-slate-800 truncate">Rovinietă / vignette</span>
        {url ? (
          <a
            href={url}
            target="_blank"
      ...
      <main className="flex-1 min-h-0 flex flex-col">
        ...
        {!loading && !error && !payload?.active && (
          <p className="p-4 text-slate-600">You need an active reservation to view the vignette file for your vehicle.</p>
        )}
        {!loading && !error && payload?.active && !url && (
          <p className="p-4 text-slate-600">No vignette file has been uploaded for this vehicle yet.</p>
        )}
        {!loading && !error && url && mode === "pdf" && (
          <iframe title="Vignette PDF" src={url} className="flex-1 w-full min-h-[75vh] border-0 bg-slate-200" />
        )}
        {!loading && !error && url && mode === "image" && (
          <div className="flex-1 overflow-auto flex items-start justify-center p-4 bg-slate-900">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={url} alt="Vignette document" className="max-w-full w-auto h-auto object-contain" />
          </div>
        )}
        {!loading && !error && url && mode === "embed" && (
          <iframe title="Vignette document" src={url} className="flex-1 w-full min-h-[75vh] border-0 bg-white" />
        )}
      </main>
    </div>
  );
